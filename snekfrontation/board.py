...
class Space:
    ...
    def get_piece_named(self, piece_name) -> Optional[Piece]:
        try:
            return next(piece for piece in self.pieces if piece.name == piece_name)
...
DEFAULT_BOARD_SPACES = [
    [Space("Mordor", 4)],
    [Space("Dagorlad", 2), Space("Gondor", 2)],
    [Space("Mirkwood", 2), Space("Fangorn", 2), Space("Rohan", 2)],
    [
        Space("Hoher Pass", 1),
    ...
    [Space("Rhudaur", 2), Space("Eregoin", 2), Space("Enedwaith", 2)],
    [Space("Arthedain", 2), Space("Cardolan", 2)],
    [Space("Alienland", 2)],
]
...
class Move:
    ...
    def __init__(self, player: Player, piece: Piece, src: Coordinate, dst: Coordinate):
        self.player = player
        self.piece = piece
        self.src = src
        self.dst = dst
...
class Board:
    ...
    def is_move_attack(self, move: Move) -> bool:
        ...
        dst_space = self.get_space(move.dst)
        return dst_space.allegiance and not isinstance(
            move.player, dst_space.allegiance
        )

    def valid_moves_from(
        self, player: Player, src: Coordinate, piece: Piece
    ...
    def find_space_for_piece(self, piece_name: str) -> Optional[Space]:
        """
